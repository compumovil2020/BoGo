...
public class PermissionsManager
{
    ...
    public static final int READ_STORAGE_PERMISSION_FOR_MAP = 33;
    ...
    public static void requestPermissionforFragment(Fragment myself, Activity context, String permiso, String justificacion, int idCode)
    {
        // Here, thisActivity is the current activity
        if (ContextCompat.checkSelfPermission(context, permiso) != PackageManager.PERMISSION_GRANTED)
        {
            // Should we show an explanation?
            if( myself.shouldShowRequestPermissionRationale(permiso) )
            {
                Toast.makeText(context, justificacion, Toast.LENGTH_LONG).show();
            }
            // request the permission.
            myself.requestPermissions(new String[]{permiso}, idCode);
            // MY_PERMISSIONS_REQUEST_READ_CONTACTS es una
            // constante definida en la aplicación, se debe usar
            // en el callback para identificar el permiso
        }
